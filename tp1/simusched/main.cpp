...
string one_line_summary(const Settings &s) {
    ostringstream os;
    os << s.tasks_file << " " << s.num_cores << " " << s.switch_cost << " "
       << s.migrate_cost << " " << s.sched_class;
    vector<int>::const_iterator it = s.sched_args.begin();
    ++it;  // Saco el primer parametro que es un agregado para que muestre los
           // cores.
...
const char *USAGE =
    " [-h] [-v] [-o output] tasks_file num_cores switch_cost migrate_cost "
    "sched_class args\n"
    "\n"
    ...
    "\n"
    "      simusched foo.tsk 1 1 2 SchedFCFS | python graphsched.py | "
    "png_viewer\n"
...
#define _sched_create(tipo, prms)                                           \
    if (!strcmp(#tipo, sched)) {                                            \
        if (!(prms == -1 || (int)(argn.size()) == prms)) {                  \
            cerr << "error: " #tipo " recibe " << prms << " parámetro(s)." \
                 << endl;                                                   \
            return NULL;                                                    \
        }                                                                   \
        return new tipo(argn);                                              \
    }
     ...
     * parámetros que recibe su scheduler como un vector de enteros
     (vector<int>)
      o ponga -1 para una cantidad de parámetros arbitraria. */
    _sched_create(SchedFCFS, -1) _sched_create(SchedRR, -1)
        _sched_create(SchedSJF, -1) _sched_create(SchedRSJF, -1)
            _sched_create(SchedMFQ, -1) return NULL;
}
...
int main(int argc, char *argv[]) {
    ...
    if (!scheduler) {
        ...
        for (int j = 0; j < (int)settings.sched_args.size(); j++)
            cerr << (j ? ',' : '(') << settings.sched_args[j];
        if (!settings.sched_args.size())
            cerr << "(";
        cerr << ")" << endl;
        return 3;
    }
    ...
    cout << "# SETTINGS " << argv[0] << " " << one_line_summary(settings)
         << endl;
