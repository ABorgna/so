...
void* task_thread(task_data* tsk) {
    ...
    //
    tsk->blk = ST_EXIT;
    _D(cerr << "TSK unlock(sched) exit" << endl;)
...
static void uso_X(task_data* tsk, enum status_t tp, unsigned int ms) {
    ...
    pthread_mutex_lock(&tsk->mutex);
    _D(cerr << "TSK lock(mutex[" << tsk - tsks << "])" << endl;)
}

// Se agrega pid para poder ver obtener
// los datos en tsks
void uso_CPU(int pid, unsigned int ms) { uso_X(&(tsks[pid]), ST_CPU, ms); }

void uso_IO(int pid, unsigned int ms) { uso_X(&(tsks[pid]), ST_IO, ms); }
...
void simulate(SchedBase& sch, std::vector<ptsk>& lote,
    ...
    forn(i, n) {
        ...
        tsks[i].blkms = 0;
        tsks[i].lastcpu = -1;
        if (pthread_create(&(tsks[i].tid), NULL, (void* (*)(void*))task_thread,
                           (void*)(&(tsks[i]))) < 0) {
            perror("Lanzando la tarea (no use muchas tareas, < 500)");
            return;
        }
    }
    ...
    for (uint32_t i = 0; i < settings.num_cores; i++) {
    ...
    while (finished < n || context_remain) {
        if (settings.verbose) {
            ...
            for (uint32_t i = 0; i < settings.num_cores; i++) {
                ...
                cerr << "cpu " << i << " pid = " << pid << " rem "
                     << contexts[i].remaining;
                if (pid != IDLE_TASK) {
                    cerr << " [" << pid << " ST:" << tsks[pid].blk
                         << " ms:" << tsks[pid].blkms << "]";
                }
                cerr << endl;
            }
        ...
        while (!unblock.empty() && unblock.top().first >= -(int)cur_time) {
            int pid = -unblock.top().second;
            unblock.pop();
            _D(cerr << "SCH unblock(" << pid << ")" << endl;)
            sch.unblock(pid);  // pid
            to_unblock.push_back(pid);
            int unblocked = 0;
            for (uint32_t i = 0; i < settings.num_cores && !unblocked; i++) {
        ...
        // Itera por cada cpu
        for (uint32_t cpu = 0; cpu < settings.num_cores; cpu++) {
            ...
            if (!cpu_context_remain) {
                ...
                if (cpu_pid == IDLE_TASK) {
                    ...
                    _D(cerr << "SCH tick( " << cpu << " ,TICK) -> " << npid
                            << endl;)
                } else {
                    if (tsks[cpu_pid].blk == ST_CPU && !tsks[cpu_pid].blkms) {
                        _D(cerr << "SCH unlock(" << cpu_pid << ") tick"
                                << endl;)
                        pthread_mutex_unlock(&(tsks[cpu_pid].mutex));
                        pthread_mutex_lock(&m_sched);
                        _D(cerr << "SCH lock(sched)" << endl;)
                    }
                    switch (tsks[cpu_pid].blk) {
                        case ST_EXIT:
                            finished++;
                            npid = sch.tick(cpu, EXIT);
                            cout << "EXIT " << cur_time << " " << cpu_pid << " "
                                 << cpu << endl;
                            _D(cerr << "SCH tick( " << cpu << " ,EXIT) -> "
                                    << npid << endl;)
                        ...
                        case ST_IO:
                            if (tsks[cpu_pid].blkms >= 0) {
                                unblock.push(
                                    make_pair(-(cur_time + tsks[cpu_pid].blkms),
                                              -cpu_pid));
                            ...
                            cout << "BLOCK " << cur_time << " " << cpu_pid
                                 << endl;
                            npid = sch.tick(cpu, BLOCK);
                            _D(cerr << "SCH tick( " << cpu << " ,BLOCK) -> "
                                    << npid << endl;)
                            break;
                        case ST_CPU:
                            if (tsks[cpu_pid].blkms) {
                                ...
                                cerr << "FATAL ERROR, this should not happend"
                                     << endl;
                            }
                            npid = sch.tick(cpu, TICK);
                            _D(cerr << "SCH tick( " << cpu << " ,TICK) -> "
                                    << npid << endl;)
                            break;
                    }
                }
                if (npid == IDLE_TASK) {
                    // cerr << "SCH unlock(sched)" <<
                    // endl;pthread_mutex_unlock(&m_sched);
                } else {
                    if (npid < 0 || npid >= n) {
                        cerr << "Error!, scheduler sent an invalid pid=" << npid
                             << endl;
                        return;
                    }
                    if (!tsks[npid].running) {
                ...
                if (cpu_pid != npid) {
                    if (npid != IDLE_TASK) {
                        ...
                        if (cpu != (uint32_t)tsks[npid].lastcpu &&
                            tsks[npid].lastcpu != -1) {
        ...
        for (int j = 0; j < (int)to_unblock.size(); j++)
            cout << "UNBLOCK " << cur_time << " " << to_unblock[j] << endl;
        context_remain = 0;
        ...
        for (uint32_t i = 0; i < contexts.size(); i++) {
            ...
            if (contexts[i].remaining /*context_remain*/) {
                ...
                cout << "CPU " << cur_time << " " << contexts[i].pid << " " << i
                     << endl;
            }
        }
        ...
        forn(i, dlote.size()) {
            if (dlote[i].first == cur_time)
                cout << "DEADLINE " << cur_time << " " << dlote[i].second
                     << endl;
        }

        cur_time++;
    }
    forn(i, n) { pthread_join(tsks[i].tid, NULL); }
    free(tsks);
}
