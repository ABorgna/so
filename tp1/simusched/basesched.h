...
class SchedBase {
   public:
    virtual ~SchedBase(){};
    /* Constructor, recibe los parámetros pasados al scheduler como una lista de
     * enteros. */

    /* load(pid) será llamada cuando una tarea nueva (pid) es creada.
            si deadline > 0, significa que la tarea debe terminarse antes de
       deadline
    */
...
/* Factory (ignorar) */
template <typename T>
SchedBase& create_sched(const std::vector<int>& argn) {
    return (new T(argn));
}
